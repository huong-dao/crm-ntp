...
4. Redirect → `/dashboard` (hoặc `callbackUrl` nếu có, ví dụ sau khi quét QR in đơn)
...
- **Tỷ lệ thăm viếng theo tổ** (bảng):
  - `totalHouseholdsPerTeam`: số hộ phụ trách (distinct hộ từ thành viên thuộc tổ)
  - `completedVisitCount / totalHouseholdsPerTeam`: số đơn trạng thái **Hoàn thành** của tổ
  - `visitedHouseholdCount / totalHouseholdsPerTeam`: số hộ có ít nhất một đơn **Hoàn thành** (distinct `household_id`)
...
- Button: [Export Excel]
- Button: [Tải file mẫu] — trên header, cạnh Import (không trong popup)
- Button: [Import Excel]
...
- **Tải file mẫu:** nút trên header trang `/members` (cùng hàng với Import), không nằm trong popup
- File mẫu có **đầy đủ cột** như khi import (28 cột — xem `MEMBER_IMPORT_TEMPLATE_HEADERS`)
- Tự tạo **hộ gia đình**, **tổ thăm viếng**, **ban ngành** nếu mã/tên chưa tồn tại
- **Thứ tự khuyến nghị:** import thành viên **trước** import tổ thăm viếng
...
### Export (`Export Excel` trên `/members`)
- Xuất theo filter hiện tại
- Cột khớp file mẫu import (đầy đủ dữ liệu thành viên)
...
### Export (`Export Excel` trên `/households`)
- Nút trên header trang
- Cột: Mã hộ | Mã tín hữu chủ hộ | Tên chủ hộ | Số thành viên
- Xuất theo bộ lọc tìm kiếm hiện tại
- Hộ được tạo khi import thành viên (cột Mã hộ) — **không có import riêng cho hộ**
...
### Header trang
- [Export Excel] [Tải file mẫu] [Import Excel] [+ Thêm tổ]
...
### Logic
- Upload `.xlsx`
- **Tải file mẫu:** nút trên header (không trong popup import)
- **Mã tổ thăm viếng** bắt buộc; chưa có → tạo tổ mới; đã có → cập nhật
- **Mã tín hữu**: trưởng tổ (member phải tồn tại nếu có giá trị — import thành viên trước)
- **Khu vực phụ trách** bắt buộc
- Báo cáo từng dòng OK/lỗi sau import

### Export (`Export Excel` trên `/visit-teams`)
...
- **Góc trên bên trái:** mã QR trỏ tới `/visit-requests/[id]/edit` (URL tuyệt đối theo `NEXTAUTH_URL`)
- Quét QR khi chưa đăng nhập → `/login?callbackUrl=...` → sau login redirect về trang sửa đơn
...
### Export (`Export Excel` trên `/visit-requests`)
- Nút trên header trang
- Xuất theo filter hiện tại
- Cột: Mã đơn | Lịch thăm viếng | Ngày thăm thực tế | Tình trạng | Mã hộ | Mã tổ thăm viếng | Mã tín hữu đại diện | Nhân sự | Nội dung
